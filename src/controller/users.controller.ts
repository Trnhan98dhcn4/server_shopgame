        ...
        await usersIdentity
            ...
            .then((response) => {
                if (response) {
                    ...
                    response.pricePrev = pricePrev;

                    response.historyUser = body.historyUser.map((item) => ({
                        img1: item.img1,
                        price: item.price,
                        SL: item.SL,
                        title: item.title,
                    ...
                    throw createHttpError(404, "Error Update");
                }
            })
            .catch((error) => next(error));
    };
    postUserAccount = async (
        req: Request,
        res: Response,
        next: NextFunction
    ) => {
        ...
        const id = req.params.id;
        try {
            const existingUser = await usersIdentity.findById(id);

            if (existingUser) {
                return res.status(409).json({ message: "User already exists" });
            }

            const newUser = await usersIdentity.create(body);
            res.status(201).json(newUser);
        } catch (error) {
            next(error);
        }
